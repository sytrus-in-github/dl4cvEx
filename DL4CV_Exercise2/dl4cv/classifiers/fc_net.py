...
class TwoLayerNet(object):
    ...
    def loss(self, X, y=None):
        ...
        ############################################################################
        Z, cache1 = affine_forward(X, self.params['W1'], self.params['b1'])
        Z, cache2 = relu_forward(Z)
        scores, cache3 = affine_forward(Z, self.params['W2'], self.params['b2'])
        ############################################################################
        #                             END OF YOUR CODE                             #
        ############################################################################
...
class FullyConnectedNet(object):
    ...
    def __init__(self, hidden_dims, input_dim=3 * 32 * 32, num_classes=10,
        ...
        self.params['W'+str(len(dims))] = np.random.normal(0., weight_scale, [dims[len(dims)-1], num_classes])
        if use_batchnorm:
            for i in range(1, len(dims)):
    ...
    def loss(self, X, y=None):
        ...
        ############################################################################
        loss, dout = softmax_loss(scores, y)
        loss += 0.5 * self.reg * math.fsum([np.sum(c[1]**2) for c in affine_caches])
        dout, grads['W'+str(h+1)], grads['b'+str(h+1)] = affine_backward(dout, affine_caches[h])
        grads['W'+str(h+1)] += self.reg * affine_caches[h][1]
        for i in xrange(h-1, -1, -1):
            if self.use_dropout:
                dout = dropout_backward(dout, dropout_caches[i])
            dout = relu_backward(dout, relu_caches[i])
            if self.use_batchnorm:
                dout, grads['gamma'+str(i+1)], grads['beta'+str(i+1)] = batchnorm_backward(dout, bn_caches[i])
            dout, grads['W'+str(i+1)], grads['b'+str(i+1)] = affine_backward(dout, affine_caches[i])
            grads['W'+str(i+1)] += self.reg * affine_caches[i][1]
        ############################################################################
        #                             END OF YOUR CODE                             #
        ############################################################################
